...
description: >-
  ...
  routed here or the user names this skill with $dev already loaded. TypeScript
  deltas: named types over string/object soup, green≠correct, one runtime truth.
---

# TypeScript Dev

Follow `$dev` for classify, shared stance, API truth / Dash recipe, workflow/plan, validation law, phase commits, and handoff skeleton. This pack adds TypeScript deltas only. Craft (modules, types, measured perf) lives in `architecture` — do not inline it here.
...
- Evidence: touched modules, public contracts, real call sites, existing tests, generated vs hand types, compat posture (ask if unclear).
