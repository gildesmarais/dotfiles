...
5. **Classify (Shift-Left Architecture Enforcement):** `surgical` | `design` | `review-hand-off`.
   ...
   - **Gated Rule:** When `design` is earned and approved-plan inheritance does not apply, DO NOT edit code immediately. You MUST invoke `architecture` (load its axioms and matched references) and document the structural/seam decisions in the plan or milestone ledger before writing code.
