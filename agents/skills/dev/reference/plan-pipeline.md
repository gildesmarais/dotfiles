...
5. Architecture pass when `design` is earned; write its outcome into each affected phase's `decisions`.
...
```yaml
target_files: [path/to/exact-file]
read_context: [path/to/read-only-file]
depends_on: []
verification_gate: "<exact repo-native command>"
commit_message: "<type>(<scope>): <description>"
# optional — required when design earned:
# decisions: "<chosen seam/contract>; rejected <alternative> because <why>"
```
````

Carrier laws:

- Each `## Phase NN — <outcome>` heading is followed by exactly one `yaml` fence with the five required keys, plus an optional `decisions` key (required on every phase touched by an earned `design` pass; omit otherwise).
...
- [ ] Every phase block parses as YAML with exact paths; phases independent of each other have disjoint `target_files`
...
- **Multi-agent** — one Cursor agent per phase. Prompt is the plan path, the phase id, and `orchestrated: true`. Unmet `depends_on` waits. Disjoint ready phases run together. This session does not implement.
