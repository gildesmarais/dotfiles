...
| ------------ | -------------------------------- | --------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------- |
| **Intent**   | Fuzzy need → actionable work     | `triage`; `prompt-compiler`; `jira-ticket`                                                                                  | `triage`: `intake` (+ playbooks); `prompt-compiler`: `compile` \| `run`; Jira → `jira-ticket`                                           |
...
1. **Product before non-trivial scope** — `jira-ticket` / feature asks load `product-owner` **`gate`**. Impl continues only on **Build Now** → `$dev` only (classifies; loads `architecture` when design). Skip for pure bug fix, refactor, infra. Product is gate-only this pass (stubs unauthored). Incident/observability/reproduce **without** Jira or IR → **`triage` first** (ledger → PO and/or `$dev` `plan` per class). Jira entry stays `jira-ticket`.
