...
- **`todo`** — Typing `todo` opens the filtered list (mirrors `todo list --json --lookback-days 28`). Press `↩` to mark done, `⌘O` to open in `$EDITOR`. Typing `todo add Buy milk #errands` adds the task instantly before the list renders.
...
- Optionally set `TODO_RAYCAST_LOOKBACK_DAYS` to change the default 28-day window shown in the list.
